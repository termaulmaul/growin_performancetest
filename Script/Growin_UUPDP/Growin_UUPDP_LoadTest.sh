...
# chmod +x Growin_UUPDP_LoadTest.sh
# ./Growin_UUPDP_LoadTest.sh
# Declare variables
ENV="INT"
USER=40
DURATION="2h"
...
../../k6 run Growin_UUPDP.js -e RUNBY="$RUNBY" -e ENV="$ENV" -e USER="$USER" -e DURATION="$DURATION" -e NUMSTART="$NUMSTART" -e SCENARIO=BP001 -e PLATFORM=Web --out dashboard=export=../../Report/Growin_UUPDP/Web/${RUNTYPE}/"$RUNBY"_"$RUNTYPE"_${dateStr}_${timeStr}.html
sleep "$SLEEP"