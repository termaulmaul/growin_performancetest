 ...
 * Original: Script/Growin_Eipo_Stock/iOS/BP008.js
 *
...
check(response, {
                ...
                if (`${__ENV.ENV}` != 'INT') {
                    console.error(`${email} ERROR Delete Order || Status: ${response.status} || Response Body: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            }
        });
    }
    ...
    if (token && orderID) {
        ...
        responses.forEach((response, index) => {
            ...
            if (response.status === 200) {
                ...
                if (`${__ENV.ENV}` != 'INT') {
                    console.error(`${email} ERROR Get Orders || Status: ${response.status} || Response Body: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            }
        });
    }
