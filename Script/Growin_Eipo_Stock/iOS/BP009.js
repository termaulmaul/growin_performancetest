...
// ##READ ME
//BP009 - e-IPO Withdraw Order Screen
//RUN QA : ../../../k6 run BP009.js -e RUNBY=Manual -e ENV=QA -e USER=10 -e DURATION=1m -e NUMSTART=98 --out dashboard=export=../../../Report/Growin_Eipo_Stock/iOS/BP009/Manual/Manual_DryRun_1029_1343_BP009.html
//RUN INT: ../../../k6 run BP009.js -e RUNBY=Manual -e ENV=INT -e USER=205 -e DURATION=5m -e NUMSTART=101 --out dashboard=export=../../../Report/Growin_Eipo_Stock/iOS/BP008/Manual/Manual_DryRun_1112_1426_BP008.html
//RUN STRESS TEST: ../../k6 run BP009.js -e RUNBY=Manual -e ENV=INT -e NUMSTART=0 --out dashboard=export=../../../Report/Growin_Eipo_Stock/iOS/BP009/Manual/Manual_DryRun_2021_1128_BP009.html
...
export default function (data) {
    ...
    let res = http.post(base_url + '/eipo/mock/order', EIPO_Order_Payload, { headers: orderHeaders });
    ...
    if (token && orderID) {
        const urls = [
            base_url + `/eipo/mock/order/${orderID}`,
        ];
        ...
        const requests = [
            ['DELETE', urls[0], null, { headers: stepOneHeaders }],
        ];
        const responses = http.batch(requests);

        responses.forEach((response, index) => {
            const metrics = [
                WithdrawOrderScreen.Delete_Eipo_Order,
            ];
    ...
    if (token && orderID) {
        const urls = [
            base_url + `/eipo/mock/order?page=1&per_page=20`,
        ];
        ...
        responses.forEach((response, index) => {
            const metrics = [
                WithdrawOrderScreen.Eipo_Order
            ];
...
export function handleSummary(data) {
    try {
        ...
        if(`${__ENV.RUNBY}`=='Manual'){
            const htmlPath = `../../../Report/Growin_Eipo_Stock/iOS/BP009/Manual/${__ENV.RUNBY}_Detail_BP009_${dateStr}_${timeStr}.html`;
            ...
            const htmlPath = `../../../Report/Growin_Eipo_Stock/iOS/BP009/Regression/${__ENV.RUNBY}_Detail_BP009_${dateStr}_${timeStr}.html`;
            console.log(`Generating HTML: ${htmlPath}`);
            ...
            const htmlPath = `../../../Report/Growin_Eipo_Stock/iOS/BP009/LoadTest/${__ENV.RUNBY}_Detail_BP009_${dateStr}_${timeStr}.html`;
            console.log(`Generating HTML: ${htmlPath}`);
