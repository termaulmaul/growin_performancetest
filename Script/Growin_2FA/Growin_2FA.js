// Command
// Run Multiple BP
// ../../k6 run Growin_2FA.js -e RUNBY=LoadTest -e ENV=INT -e USER=250 -e DURATION=5m -e NUMSTART=101 -e PLATFORM=Web  --out dashboard=export=../../Report/Growin_2FA/Web/LoadTest/Manual_LoadTest_0507_1701.html

// Run Single BP Web
// ../../k6 run Growin_2FA.js -e RUNBY=Manual -e ENV=INT -e USER=1000 -e DURATION=5m -e NUMSTART=1 -e SCENARIO=BP001 -e PLATFORM=Web --out dashboard=export=../../Report/Growin_2FA/Web/BP001/Manual/Manual_DryRun_0507_1653_BP001.html
...
// ../../k6 run Growin_2FA.js -e RUNBY=Manual -e ENV=INT -e USER=1 -e DURATION=15m -e NUMSTART=1001 -e SCENARIO=BP003 -e PLATFORM=Web --out dashboard=export=../../Report/Growin_2FA/Web/BP003/Manual/Manual_DryRun_0507_2154_BP003.html
...
const BP_CONFIG = {
    Web: {
        ...
        BP003: { fn: BP003_Web, skipSetupLogin: true, numStart: 1 },
    },
    // iOS: {
...
export function setup() {
    ...
    // /* log disabled */ (`📱 Platform: ${platform}`);
    // /* log disabled */ (`🔢 Mode: ${isMultiBP ? 'Multi-BP (LoadTest) — pakai numStart dari BP_CONFIG' : 'Single BP (Manual) — pakai NUMSTART dari env'}`);
    ...
    selectedBPs.forEach((bp, bpIndex) => {
        ...
        //
        //    getUserCredentials(userNum, bpOffset) menghasilkan:
        //    email_index = NUMSTART_env + userNum + bpOffset - 1
        //
        //    Untuk multi-BP, kita ingin setiap BP menggunakan numStart-nya sendiri
        //    sebagai basis, BUKAN NUMSTART_env global. Ini menghindari offset negatif
        //    ketika bpNumStart < NUMSTART_env (contoh: BP002.numStart=1, NUMSTART_env=101).
        //
        //    Solusi: panggil getUserCredentials dengan bpOffset = bpNumStart - NUMSTART_env
        //    sehingga: email_index = NUMSTART_env + userNum + (bpNumStart - NUMSTART_env) - 1
        //                          = bpNumStart + userNum - 1  ✅ selalu benar
        //
        //    Untuk token key (userKey), gunakan bpNumStart + i - 1 agar:
        //    - Selalu positif (bpNumStart >= 1)
        //    - Unik antar BP (BP001 mulai 1501, BP002 mulai 1 — tidak overlap)
        //    - Konsisten antara setup() dan runtime BP function
        const bpNumStart = (bpConfig.numStart != null && isMultiBP) ? bpConfig.numStart : NUMSTART_env;
        // bpOffset untuk getUserCredentials: membuat email_index = bpNumStart + userNum - 1
        const bpOffset = bpNumStart - NUMSTART_env;
        // userKeyBase: basis untuk token key — selalu positif dan unik per-BP
        const userKeyBase = bpNumStart - 1; // userKey = userKeyBase + i → mulai dari bpNumStart

        // /* log disabled */ (`\n📦 Processing ${bp} on ${platform} - ${usersForThisBP} users (VU ${globalVuOffset} to ${globalVuOffset + usersForThisBP - 1})...`);
        // /* log disabled */ (`   🔢 numStart efektif: ${bpNumStart} | bpOffset: ${bpOffset} | userKey range: ${userKeyBase + 1} - ${userKeyBase + usersForThisBP}`);
        ...
        for (let localUserIndex = 1; localUserIndex <= usersForThisBP; localUserIndex++) {
            ...
            vuMapping[vuId] = {
                bp: bp,
                userKey: userKeyBase + localUserIndex  // ✅ selalu positif, unik per-BP
            };
        }
        ...
        if (skipSetupLogin) {
            for (let i = 1; i <= usersForThisBP; i++) {
                const credentials = getUserCredentials(i, bpOffset);  // ✅ bpOffset mengarahkan ke email yang benar
                const userKey = userKeyBase + i;                       // ✅ selalu positif
                tokens[userKey] = {
                    email: credentials.email,
                    token: null,
                    pin_token: null,
