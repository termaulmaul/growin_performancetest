 ...
 * Original: Script/Growin_2FA/Web/BP003.js
 *
...
console.log(`${email} ${url} || Status: ${response.status} || Body: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            } else {
    ...
    let deviceIdToDelete = null;
    {
        ...
        responses.forEach((response) => {
            ...
            if (response.status === 200) {
                ...
                metric.http_reqs.add(1);
                if (`${__ENV.ENV}` != 'INT') {
                    console.log(`${email} ${url} || Status: ${response.status} || Response: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            } else {
                metric.errorRate.add(true);
                metric.errorCount.add(1);
                ...
                if (`${__ENV.ENV}` != 'INT') {
                    console.error(`${email} ERROR ${url} || Status: ${response.status} || Response: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            }
        });
    }
