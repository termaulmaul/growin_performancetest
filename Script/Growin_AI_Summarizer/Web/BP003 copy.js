...
// ##READ ME
//BP003 - Homescreen
//RUN QA : ../../k6 run BP003.js -e RUNBY=Manual -e ENV=QA -e USER=10 -e DURATION=1m -e NUMSTART=100 --out dashboard=export=../../Report/Growin_AI_Summarizer/BP003/Manual/Manual_DryRun_2021_1138_BP003_Local.html
//RUN INT: ../../k6 run BP003.js -e RUNBY=Manual -e ENV=INT -e USER=700 -e DURATION=15m -e NUMSTART=0 --out dashboard=export=../../Report/Growin_AI_Summarizer/BP003/Manual/Manual_DryRun_2021_2102_BP003_Local.html
//RUN STRESS TEST: ../../k6 run BP003.js -e RUNBY=Manual -e ENV=INT -e NUMSTART=0 --out dashboard=export=../../Report/Growin_AI_Summarizer/BP003/Manual/Manual_DryRun_2021_1128_BP003_Local.html
...
// /marketdata/api/v1/gpt/user_activity_insert
...
export default function (data) {
    ...
    // Get user profile
    const profileTradingHeaders = {
        'Content-Type': 'application/json',
        'Cookie': `ACCESS_TOKEN=${token}`
    };
    
    let resProfileTrading = http.get(base_url + '/user/api/v1/profile/trading', { headers: profileTradingHeaders });
    ...
    if (resProfileTrading.status === 200) {
        ...
        clientID = resProfileTrading.json().data.client_id
        SID = resProfileTrading.json().data.sid
        KseiAccNo = resProfileTrading.json().data.ksei_acc_no
        AccountName = resProfileTrading.json().data.account_name
        if (`${__ENV.ENV}` != 'INT') {
    ...
    if (token) {
        const urls = [
            base_url + `/marketdata/api/v1/gpt/user_activity_insert`,
        ];
        ...
        const userActivityInsert_Payload = JSON.stringify({
            "client_id": clientID,
            "client_name": AccountName,
            "user_id": userID,
            "sid": SID,
            "ksei_account": KseiAccNo,
        ...
        const requests = [
            ['POST', urls[0], userActivityInsert_Payload, { headers: stepOneHeaders }],
        ];
        const responses = http.batch(requests);

        responses.forEach((response, index) => {
            const metrics = [
                Homescreen.Marketdata_Gpt_UserActivityInsert
            ];
...
export function handleSummary(data) {
    try {
        ...
        if(`${__ENV.RUNBY}`=='Manual'){
            const htmlPath = `../../Report/Growin_AI_Summarizer/BP003/Manual/${__ENV.RUNBY}_Detail_BP003_${dateStr}_${timeStr}.html`;
            ...
            const htmlPath = `../../Report/Growin_AI_Summarizer/BP003/Regression/${__ENV.RUNBY}_Detail_BP003_${dateStr}_${timeStr}.html`;
            console.log(`Generating HTML: ${htmlPath}`);
