...
# chmod +x Growin_OMO_LoadTest.sh
# ./Growin_OMO_LoadTest.sh
# Declare variables
ENV="INT"
USER=85
DURATION="2h"
...
../../k6 run Growin_OMO.js -e RUNBY="$RUNBY" -e ENV="$ENV" -e USER="$USER" -e DURATION="$DURATION" -e NUMSTART="$NUMSTART" -e PLATFORM=iOS --out dashboard=export=../../Report/Growin_OMO/iOS/${RUNTYPE}/"$RUNBY"_"$RUNTYPE"_${dateStr}_${timeStr}_48.html
sleep "$SLEEP"