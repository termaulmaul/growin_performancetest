...
# chmod +x [Template]_LoadTest.sh
# ./[Template]_LoadTest.sh
# Declare variables
ENV="INT"
USER=250
DURATION="2h"
...
../../k6 run [Template].js -e RUNBY="$RUNBY" -e ENV="$ENV" -e USER="$USER" -e DURATION="$DURATION" -e NUMSTART="$NUMSTART" -e PLATFORM=Web --out dashboard=export=../../Report/[Template]/Web/${RUNTYPE}/"$RUNBY"_"$RUNTYPE"_${dateStr}_${timeStr}_48.html
sleep "$SLEEP"