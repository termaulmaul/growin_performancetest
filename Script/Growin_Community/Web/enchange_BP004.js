 ...
 * Original: Script/Growin_Community/Web/BP004.js
 *
...
check(response, {
                ...
                if (`${__ENV.ENV}` != 'INT') {
                    ...
                    console.error(`${email} ERROR ${urls[index]} || Status: ${response.status} || Request Body: ${requestBody} || Response Body: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
                }
            }
        });
    }
    ...
    if (token) {
        ...
        if (response.status === 200) {
            ...
            console.error(`${email} ❌ Request failed with status: ${response.status} || Body: ${String(response.body).slice(0, 500)}`) /* ENHANCE-R4: body truncated */;
        }
