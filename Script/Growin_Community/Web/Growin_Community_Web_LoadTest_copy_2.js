// Command
// Run Multiple BP
// ../../../k6 run Growin_Community_Web_LoadTest_FINAL.js -e RUNBY=LoadTest -e ENV=INT -e USER=316 -e DURATION=1h --out dashboard=export=../../../Report/Growin_Community/Web/LoadTest/Manual_LoadTest_0107_1450.html

// Run Single BP
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=5m -e SCENARIO=BP001 --out dashboard=export=../../../Report/Growin_Community/Web/BP001/Manual/Manual_DryRun_0128_1616_BP001_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=5m -e SCENARIO=BP002 --out dashboard=export=../../../Report/Growin_Community/Web/BP002/Manual/Manual_DryRun_1227_1436_BP002_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=15m -e SCENARIO=BP003 --out dashboard=export=../../../Report/Growin_Community/Web/BP003/Manual/Manual_DryRun_0202_1057_BP003_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=25 -e DURATION=5m -e SCENARIO=BP004 --out dashboard=export=../../../Report/Growin_Community/Web/BP004/Manual/Manual_DryRun_0130_1436_BP004_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=15m -e SCENARIO=BP005 --out dashboard=export=../../../Report/Growin_Community/Web/BP005/Manual/Manual_DryRun_0202_1613_BP005_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=300 -e DURATION=15m -e SCENARIO=BP006 --out dashboard=export=../../../Report/Growin_Community/Web/BP006/Manual/Manual_DryRun_0203_1004_BP006_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=15m -e SCENARIO=BP007 --out dashboard=export=../../../Report/Growin_Community/Web/BP007/Manual/Manual_DryRun_0202_1151_BP007_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=75 -e DURATION=15m -e SCENARIO=BP008 --out dashboard=export=../../../Report/Growin_Community/Web/BP008/Manual/Manual_DryRun_0202_1211_BP008_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=5m -e SCENARIO=BP009 --out dashboard=export=../../../Report/Growin_Community/Web/BP009/Manual/Manual_DryRun_1227_1521_BP009_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=5m -e SCENARIO=BP010 --out dashboard=export=../../../Report/Growin_Community/Web/BP010/Manual/Manual_DryRun_1227_1552_BP010_Local.html
...
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=250 -e DURATION=5m -e SCENARIO=BP012 --out dashboard=export=../../../Report/Growin_Community/Web/BP012/Manual/Manual_DryRun_1229_1801_BP012_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=25 -e DURATION=5m -e SCENARIO=BP013 --out dashboard=export=../../../Report/Growin_Community/Web/BP013/Manual/Manual_DryRun_0129_1825_BP013_Local.html
// ../../../k6 run Growin_Community_Web_LoadTest_copy_2.js -e RUNBY=Manual -e ENV=INT -e USER=50 -e DURATION=5m -e SCENARIO=BP014 --out dashboard=export=../../../Report/Growin_Community/Web/BP014/Manual/Manual_DryRun_0129_1822_BP014_Local.html
...
const BP_CONFIG = {
    ...
    BP008: { percentage: 0.25, pool: 'MIXED' },
...
class UserPoolManager {
    ...
    _allocateFromPool(poolName, count, bp) {
        ...
        console.log(`   🔍 DEBUG: Allocating ${count} users from ${poolName} (${pool.start}-${pool.end}) for ${bp}`);
        ...
        if (allocated.length > 0) {
            const userList = allocated.map(u => u.userNum).join(', ');
            console.log(`   ✅ DEBUG: Allocated for ${bp} from ${poolName}: [${userList}]`);
        }
...
function calculateUserDistribution(totalUsers, selectedBPs) {
    ...
    selectedBPs.forEach((bp, index) => {
        ...
        if (index === selectedBPs.length - 1) {
            ...
            const users = Math.max(1, Math.floor((percentage / totalPercentage) * totalUsers));
...
function loginWithRetry(base_url, credentials, vuId, userNum, poolType) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
        ...
        if (loginRes.status === 200) {
            if (attempt > 1) {
                console.log(`   ✅ VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) LOGIN SUCCESS on attempt ${attempt}`);
            }
            return {
                success: true,
        ...
        if (attempt < MAX_RETRY_ATTEMPTS) {
            console.warn(`   ⚠️  VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) LOGIN attempt ${attempt}/${MAX_RETRY_ATTEMPTS} FAILED - Status: ${loginRes.status}, retrying...`);
            sleep(RETRY_DELAY);
        } else {
            console.error(`   ❌ VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) LOGIN FAILED after ${MAX_RETRY_ATTEMPTS} attempts - Status: ${loginRes.status}`);
        }
    }
...
function pinLoginWithRetry(base_url, token, credentials, vuId, userNum, poolType) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
        ...
        if (pinRes.status === 200) {
            if (attempt > 1) {
                console.log(`   ✅ VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) PIN SUCCESS on attempt ${attempt}`);
            }
            return {
                success: true,
        ...
        if (attempt < MAX_RETRY_ATTEMPTS) {
            console.warn(`   ⚠️  VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) PIN attempt ${attempt}/${MAX_RETRY_ATTEMPTS} FAILED - Status: ${pinRes.status}, retrying...`);
            sleep(RETRY_DELAY);
        } else {
            console.error(`   ❌ VU${vuId} (${credentials.email}, User${userNum}, Pool:${poolType}) PIN FAILED after ${MAX_RETRY_ATTEMPTS} attempts - Status: ${pinRes.status}`);
        }
    }
...
export function setup() {
    ...
    selectedBPs.forEach((bp) => {
        ...
        for (let batchNum = 0; batchNum < numBatches; batchNum++) {
            ...
            for (let i = batchStart; i < batchEnd; i++) {
                ...
                const loginResult = loginWithRetry(base_url, credentials, vuId, userAllocation.userNum, userAllocation.pool);
                ...
                if (loginResult.success) {
                    ...
                    const pinResult = pinLoginWithRetry(base_url, loginResult.token, credentials, vuId, userAllocation.userNum, userAllocation.pool);
    ...
    selectedBPs.forEach(bp => {
        ...
        if (bpTokens.length > 0) {
            ...
            const emails = bpTokens.map(t => t.email).sort();
            const userRange = userNums.length === 1 
                ? `${userNums[0]}`
                : `${userNums[0]}-${userNums[userNums.length - 1]}`;
            const emailRange = emails.length === 1
                ? emails[0]
                : `${emails[0]} to ${emails[emails.length - 1]}`;
            
            console.log(`   ${bp}: ${logins}/${bpTokens.length} logins, ${pins} PINs | Pool: ${pools}`);
            console.log(`      └─ Users: ${userRange} | Emails: ${emailRange}`);
        } else {
    ...
    // ✅ CREATE BP-SPECIFIC TOKEN ARRAYS FOR RUNTIME MAPPING
    const bpTokens = {};
    selectedBPs.forEach(bp => {
        bpTokens[bp] = Object.entries(tokens)
            .filter(([_, token]) => token.bp === bp)
            .map(([vuId, token]) => ({ 
                vuId: parseInt(vuId), 
                ...token 
            }))
        ...
        console.log(`🔗 BP Token Mapping: ${bp} has ${bpTokens[bp].length} tokens ready`);
    });
    ...
    return { 
        ...
        bpTokens: bpTokens  // ✅ ADD BP-specific token mapping
    };
}
