...
// ##READ ME
//BP013 - Community Detail USER TRIAL JOIN/LEAVE
//RUN QA : ../../../k6 run BP013.js -e RUNBY=Manual -e ENV=QA -e USER=1 -e DURATION=1m -e NUMSTART=9 --out dashboard=export=../../../Report/Growin_Community/Web/BP013/Manual/Manual_DryRun_1110_1125_BP013_Web_Local.html
//RUN INT: ../../../k6 run BP013.js -e RUNBY=Manual -e ENV=INT -e USER=1 -e DURATION=10m -e NUMSTART=1 --out dashboard=export=../../../Report/Growin_Community/Web/BP013/Manual/Manual_DryRun_1117_1553_BP013_Web_Local.html
//RUN STRESS TEST: ../../../k6 run BP013.js -e RUNBY=Manual -e ENV=INT -e NUMSTART=0 --out dashboard=export=../../../Report/Growin_Community/Web/BP013/Manual/Manual_DryRun_2021_1128_BP013_Web_Local.html
...
export default function (data) {
    ...
    let communityID;
    const communityProfile_Headers = {
        // 'Cookie': `ACCESS_TOKEN=${token}`,
        // 'Content-Type': 'application/json',
    ...
    let resCommunityProfile = http.get(base_url + `/socialinvesting/api/v1/community-profile/get-profile`, { headers: communityProfile_Headers })

    if (resCommunityProfile.status === 200) {
        const CommunityProfile = resCommunityProfile.json();
        communityID = CommunityProfile.data.community_id;
    } else {
        console.error(`${email} CommunityProfile Failed - Status: ${resCommunityProfile.status} - Body: ${resCommunityProfile.body}`);
    }

    // Batch 1
    if (token) {
        const urls = [
            base_url + `/socialinvesting/api/v1/social/request-kick-member?channel_id=${channelId}`,
        ];

        const Socialinvesting_Social_RequestKickMember_Payload = JSON.stringify({
            community_ids: [channelId],
        });
...
export function handleSummary(data) {
    try {
        ...
        if(`${__ENV.RUNBY}`=='Manual'){
            const htmlPath = `../../../Report/Growin_Community/Web/BP013/Manual/${__ENV.RUNBY}_Detail_BP013_Web_${dateStr}_${timeStr}.html`;
            ...
            const htmlPath = `../../../Report/Growin_Community/Web/BP013/Regression/${__ENV.RUNBY}_Detail_BP013_Web_${dateStr}_${timeStr}.html`;
            console.log(`Generating HTML: ${htmlPath}`);
            ...
            const htmlPath = `../../../Report/Growin_Community/Web/BP013/LoadTest/${__ENV.RUNBY}_Detail_BP013_Web_${dateStr}_${timeStr}.html`;
            console.log(`Generating HTML: ${htmlPath}`);
