 ...
 * Original: Script/Growin_Community/Web/BP005.js
 *
    ...
    if (token) {
        ...
        responses.forEach((response, index) => {
            ...
            if (response.status === 200) {
                ...
                metric.http_reqs.add(1);
                if (`${__ENV.ENV}` != 'INT') {
                    console.log(`${email} ${urls[index]} || Status: ${response.status} || Body: ${String(response.body).slice(0, 500)} || UUID : ${switchChannelID}`) /* ENHANCE-R4: body truncated */;
                }
            } else {
                metric.errorRate.add(true);
                metric.errorCount.add(1);
                ...
                if (`${__ENV.ENV}` != 'INT') {
                    const requestBody = requests[index][2];
                    console.error(`${email} ERROR ${urls[index]} || Status: ${response.status} || Response Body: ${String(response.body).slice(0, 500)} || Request Body: ${requestBody} || UUID : ${switchChannelID}`) /* ENHANCE-R4: body truncated */;
                }
            }
        });
    }
