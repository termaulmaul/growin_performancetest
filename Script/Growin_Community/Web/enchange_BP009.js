 ...
 * Original: Script/Growin_Community/Web/BP009.js
 *
...
export function BP009(data) {
    const scenarioName = 'BP009';
    const base_url = data.base_url;
    const isIntEnv = `${__ENV.ENV}` === 'INT';
    
    // ✅ GET CORRECT TOKEN FROM BP-SPECIFIC ARRAY
    const bp009Tokens = data.bpTokens[scenarioName];
    if (!bp009Tokens || bp009Tokens.length === 0) {
    ...
    const tokenIndex = iterationIndex % bp009Tokens.length; // Wrap around if iterations > tokens
    
    const userToken = bp009Tokens[tokenIndex];
    ...
    if (token) {
        const urls = [
            base_url + `/socialinvesting/api/v1/social/messages-history?channel_id=${channel_id}&message_limit=2`,
        ];
