...
const CommunityMemberKickRequestApproval = {
    Socialinvesting_Social_GetKickRequest: {
        errorCount: new Counter("error_count_Socialinvesting_Social_GetKickRequest"),
        errorRate: new Rate("error_rate_Socialinvesting_Social_GetKickRequest"),
        httpDuration: new Trend("duration_Socialinvesting_Social_GetKickRequest"),
        httpWaiting: new Trend("waiting_Socialinvesting_Social_GetKickRequest"),
        requestRate: new Counter("rps_Socialinvesting_Social_GetKickRequest"),
        http_reqs: new Counter("sample_Socialinvesting_Social_GetKickRequest"),
    },
    Socialinvesting_Social_UpdateKickRequest: {
        errorCount: new Counter("error_count_Socialinvesting_Social_UpdateKickRequest"),
        errorRate: new Rate("error_rate_Socialinvesting_Social_UpdateKickRequest"),
        httpDuration: new Trend("duration_Socialinvesting_Social_UpdateKickRequest"),
        httpWaiting: new Trend("waiting_Socialinvesting_Social_UpdateKickRequest"),
        requestRate: new Counter("rps_Socialinvesting_Social_UpdateKickRequest"),
        http_reqs: new Counter("sample_Socialinvesting_Social_UpdateKickRequest"),
    },
...
export function BP011(data) {
    const scenarioName = 'BP011';
    const base_url = data.base_url;
    const isIntEnv = `${__ENV.ENV}` === 'INT';
    const bp011Tokens = data.bpTokens[scenarioName];
    if (!bp011Tokens || bp011Tokens.length === 0) {
        console.error(`❌ ${scenarioName} - No tokens available!`);
        return;
    }
    const iterationIndex = exec.scenario.iterationInInstance;
    const tokenIndex = iterationIndex % bp011Tokens.length; // Wrap around if iterations > tokens
    
    const userToken = bp011Tokens[tokenIndex];
    if (!userToken || !userToken.token) {
        console.error(`❌ ${scenarioName} Iteration ${iterationIndex} - No valid token at index ${tokenIndex}!`);
        return;
    }
    if (userToken.pool !== 'ADMIN') {
        console.error(`❌ CRITICAL: ${scenarioName} using ${userToken.pool} user (${userToken.email}) instead of ADMIN! ABORTING.`);
        return;
    }
