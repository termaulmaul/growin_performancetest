...
export function BP004(data) {
    ...
    // ✅ Get correct user allocation from mapping
    const userMapping = data.vuMapping[vuId];
    if (!userMapping) {
        console.error(`❌ VU${vuId} - No mapping found!`);
        return;
    }
    
    const userToken = data.tokens[vuId];
    if (!userToken || !userToken.token) {
        console.error(`❌ VU${vuId} (User ${userMapping.userNum}, Pool: ${userMapping.pool}) - No token!`);
        return;
    }
    
    console.log(`🔍 VU${vuId} using User${userMapping.userNum} from ${userMapping.pool} pool`);
