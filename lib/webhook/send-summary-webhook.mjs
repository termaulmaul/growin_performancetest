...
if (type === 'teams') {
  ...
  let html = `<b>🧪 PT Run Report — ${result.suite || 'Direct Run'}</b>\n`;
  
  let statusEmoji = avgOk && errOk && rpsOk ? '✅' : (avgOk && errOk && !rpsOk ? '⚠️' : '❌');
  html += `<b>Status:</b> ${statusEmoji} ${statusText.replace(/^##\s*/, '')}\n`;
  html += `<b>Reason:</b> ${reason}\n`;
  html += `<b>Target:</b> ${targetUrl}\n`;
  ...
  if (topErrors.length > 0) {
    ...
    for (const e of topErrors) {
      html += `• [${e.status}] ${e.endpoint} — ${e.count}×\n`;
    }
    html += `\n`;
  }

  if (utilizationUrl) {
    html += `<a href="${utilizationUrl}">📈 View Utilization Report</a>`;
  }
