...
if (type === 'teams') {
  const cardBody = [
    ...
    },
    {
      ...
      "text": "```\n" +
        ...
        `${totalReqs}\t${overallAvg.toFixed(2)}ms ${avgOk ? "✓" : "✗"}\t${p95All.toFixed ? p95All.toFixed(2) : p95All}ms ${avgOk ? "✓" : "✗"}\t${overallErr.toFixed(2)}% ${errOk ? "✓" : "✗"}\t${totalRpsVal.toFixed(2)} ${rpsOk ? "✓" : "✗"}\t${avgTps.toFixed(2)} ${rpsOk ? "✓" : "✗"}\n` +
        "```",
      "wrap": true
    }
  ];
  ...
  if (apiRows.length > 0) {
    ...
    // Build TSV Table block
    let tsvTable = "#\tAPI\tSamp\tAvg\tP95\tErr%\tRPS\tErr\n";
    ...
    apiRows.forEach((api, idx) => {
      ...
      tsvTable += `${idx + 1}\t${api.name.replace(/_/g, ' ')}\t${samples}\t${(api.dur_avg || 0).toFixed(2)}\t${(api.dur_p95 || 0).toFixed(2)}\t${errRate}%\t${rps}\t${errExact}\n`;
    });

    // TOTAL row
    const totalRps = durS > 0 ? (totalSamples / durS).toFixed(2) : "0";
    tsvTable += `\tTOTAL\t${totalSamples}\t${overallAvg.toFixed(2)}\t${p95All.toFixed ? p95All.toFixed(2) : p95All}\t${overallErr.toFixed(2)}%\t${totalRps}\t${totalErrSamples}\n`;
  ...
  if (apiRows.length > 0) {
    ...
    for (const r of apiRows) {
      let n = r.name.replace(/^BP\d+_|001_01_/g,'').substring(0,13).padEnd(13);
      const samp = String(r.samples||0).padStart(4);
      const avg = (r.dur_avg||r.dur_p95||0).toFixed(0).padStart(4);
      const p95 = (r.dur_p95||0).toFixed(0).padStart(4);
      const err = (r.err_rate!=null?r.err_rate*100:0).toFixed(1).padStart(4) + '%';
      const rps = (durS > 0 ? ((r.samples||0)/durS).toFixed(0) : '0').padStart(4);
