...
if (type === 'teams') {
  const cardBody = [
    ...
    },
    {
      "type": "FactSet",
      "facts": [
        { "title": "Samples", "value": String(totalReqs) },
        { "title": `P95 (<${tAvg}ms)`, "value": `${overallAvg.toFixed(2)} ms ${avgOk ? "✓" : "✗"}` },
        { "title": `Error Rate (<${tErr}%)`, "value": `${overallErr.toFixed(2)}% ${errOk ? "✓" : "✗"}` },
        { "title": `Total RPS (>=${tRps})`, "value": `${totalRpsVal.toFixed(2)} ${rpsOk ? "✓" : "✗"}` }
      ]
    }
  ];
