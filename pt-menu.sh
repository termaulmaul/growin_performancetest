  ...
  if $do_check; then
    if [[ ! -f /tmp/.k6_target_check_pid ]] || ! kill -0 $(cat /tmp/.k6_target_check_pid 2>/dev/null) 2>/dev/null; then
      (
        ...
        # Check Onprem
        if [[ -n "$pass" ]]; then
          remote_k6=$(timeout 5s sshpass -p "$pass" ssh -q -o StrictHostKeyChecking=no -o ConnectTimeout=5 \
            -o ProxyCommand="sshpass -p \"$pass\" ssh -q -o StrictHostKeyChecking=no -W %h:%p qa@10.82.15.72" \
            qa@10.184.120.48 'ps auxww' 2>/dev/null | grep "[k]6 run" | grep -v "grep" | grep -v "bash -c" | head -n 1)
        fi
        # Check Oncloud
        if [[ -z "$remote_k6" ]]; then
          remote_k6=$(timeout 3s gcloud compute ssh vm-pt-ksix-0 --tunnel-through-iap --project compute-pt --zone asia-southeast2-c --command="ps auxww" 2>/dev/null | grep "[k]6 run" | grep -v "grep" | grep -v "bash -c" | head -n 1 || true)
        fi
        
        if [[ -n "$remote_k6" ]]; then
          local script_name=""
          script_name=$(echo "$remote_k6" | awk '{for(i=1;i<=NF;i++) if($i ~ /\.js$/ && $i !~ /=/ && $i !~ /^-/) print $i}' | awk -F'/' '{print $NF}' | head -n 1)
...
Press Enter...'; continue
        ...
        [[ -z "$n_user" ]] && continue
        printf "  Display Name : "; read -r n_display
        [[ -z "$n_display" ]] && n_display="$n_user"
        printf "  Password     : "; read -rs n_pwd; echo ""
        [[ -z "$n_pwd" ]] && continue
        printf "  Confirm Pwd  : "; read -rs n_pwd2; echo ""
        if [[ "$n_pwd" != "$n_pwd2" ]]; then
        ...
        [[ -z "$n_role" ]] && continue
        local add_raw add_out add_rc
        add_raw=$(echo "$n_pwd" | python3 "$PROJECT_DIR/bin/pt-usermgmt" create --by "$PT_USER" --username "$n_user" --role "$n_role" --display "$n_display" --password-stdin 2>&1)
        add_rc=$?
        add_out=$(echo "$add_raw" | python3 -c "import json,sys; raw=sys.stdin.read().strip(); raw=raw[raw.find('{'):] if '{' in raw else '{}'; d=json.loads(raw) if raw else {}; print(d.get('message', d.get('error','?')))")
        if [[ $add_rc -eq 0 ]]; then
          echo -e "  ${GRN}✓ $add_out${RST}"
        else
          echo -e "  ${RED}✘ $add_out${RST}"
        fi
        read -r -p $'\nPress Enter...'
        ;;
      "[3] Lock/Unlock User")
...
Press Enter...'
        ;;
      "[3] Pause/Resume"*)
        local job_list; job_list=$(python3 "$PROJECT_DIR/bin/pt-scheduler" list | python3 -c "import json,sys; raw=sys.stdin.read().strip(); raw=raw[raw.find('{'):] if '{' in raw else '{}'; data=json.loads(raw) if raw else {}; [print(\"%s (%s)\" % (j['id'], j['status'])) for j in data.get('data',{}).get('jobs',[])]")
        [[ -z "$job_list" ]] && { read -r -p $'
Press Enter...'; continue; }
